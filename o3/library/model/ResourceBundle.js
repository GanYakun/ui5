...
sap.ui.define([
    ...
    function (assert, Log, formatMessage, Properties) {
        ...
        ResourceBundle.prototype.getText = function (sKey, aArgs, bIgnoreKeyFallback) {
            ...
            //xwk 对key进行处理
            if (o3Tool && o3Tool.sModuleName && o3Tool.sAppName) {
                var NA = '_NA_';
                var sKeyArr = sKey.split('.');
                if (sKeyArr.length == 1) {//xwk eg: Y
                    //sd.so_obj.NA.Y
                    var sValue = this._getTextFromProperties(o3Tool.sModuleName + '.' + o3Tool.sAppName + '.' + NA + '.' + sKeyArr[0], aArgs);
                    if (sValue != null) {
                        return sValue;
                    }
                    //sd.NA.NA.Y
                    var sValue = this._getTextFromProperties(o3Tool.sModuleName + '.' + NA + '.' + NA + '.' + sKey, aArgs);
                    if (sValue != null) {
                        return sValue;
                    }
                    //NA.NA.NA.Y
                    var sValue = this._getTextFromProperties(NA + '.' + NA + '.' + NA + '.' + sKey, aArgs);
                    if (sValue != null) {
                        return sValue;
                    }
                } else if (sKeyArr.length == 2) {//xwk eg: OrderHeader.orderId
                    //sd.so_obj.OrderHeader.orderId
                    var sValue = this._getTextFromProperties(o3Tool.sModuleName + '.' + o3Tool.sAppName + '.' + sKeyArr[0] + '.' + sKeyArr[1], aArgs);
                    if (sValue != null) {
                        return sValue;
                    }
                    //sd.NA.OrderHeader.orderId
                    var sValue = this._getTextFromProperties(o3Tool.sModuleName + '.' + NA + '.' + sKeyArr[0] + '.' + sKeyArr[1], aArgs);
                    if (sValue != null) {
                        return sValue;
                    }
                    //NA.NA.OrderHeader.orderId
                    var sValue = this._getTextFromProperties(NA + '.' + NA + '.' + sKeyArr[0] + '.' + sKeyArr[1], aArgs);
                    if (sValue != null) {
                        return sValue;
                    }
                }
            ...
            if (console && console.warn) {
                console.warn("could not find any translatable text for key '" + sKey + "' in bundle '" + this.oUrlInfo.url + "'");
            }
